...
def merge_files(img_dir, ann_dir, save_dir):
    ...
    for dir_name in img_dir_list:
        img_list = os.listdir(img_dir + '/' + dir_name)
        for file_name in img_list:
            current_file_id = file_name.split('.')[0]
            ann_file_name = current_file_id + '.json'
            src_img = img_dir + '/' + dir_name + '/' + file_name
            src_ann = ann_dir + '/' + dir_name + '/' + ann_file_name
            if "_" in current_file_id:
                copy_files(src_img, save_dir + '/' + 'images/' + file_name)
                copy_files(src_ann, save_dir + '/' + 'annotation/' + ann_file_name)
            else:
                new_file_id = 'v' + dir_name.split('_')[-1] + '_' + current_file_id
